...
import { TreasuryAIAssistant, QueryCategory, type AssistantQuery } from './treasury-ai-assistant.js';
...
describe('TreasuryAIAssistant — Sprint 11.1', () => {
  ...
  const assistant = new TreasuryAIAssistant({ apiEndpoint:'http://localhost:99999/unreachable', timeoutMs:100 });

  const baseQuery: AssistantQuery = {
    tenantId:'bank-001', userId:'trader-01',
    ...
    context:{
      ...
      lcrRatio:142.5, nsfrRatio:118.3, cet1RatioPct:13.2,
      niiYTD:48_000_000,
    },
  };
  ...
  it('classifies limit question correctly', async () => {
    const r = await assistant.ask({ ...baseQuery, question:'Which counterparties are approaching limits?' });
  ...
  it('PII redaction removes IBAN-like patterns', () => {
    const a = new TreasuryAIAssistant({ piiRedaction:true, apiEndpoint:'http://localhost:99999/', timeoutMs:50 });
...
describe('ReportBuilder — Sprint 11.2', () => {
  ...
  beforeEach(() => { builder = new ReportBuilder(); });

  it('defines a report and returns a definition with ID', () => {
    const r = builder.define({ name:'Daily Blotter', tenantId:'bank-001', createdBy:'user-01',
      template: ReportTemplate.BLOTTER, dimensions:[ReportDimension.BOOK, ReportDimension.TRADER] });
    expect(r.id).toContain('RPT-');
    expect(r.template).toBe(ReportTemplate.BLOTTER);
  });

  it('uses default metrics for BLOTTER template', () => {
    const r = builder.define({ name:'Blotter', tenantId:'bank-001', createdBy:'u1',
      template:ReportTemplate.BLOTTER, dimensions:[ReportDimension.BOOK] });
    expect(r.metrics).toContain('mtm');
    expect(r.metrics).toContain('tradeId');
  });

  it('allows custom metrics override', () => {
    const r = builder.define({ name:'Custom', tenantId:'bank-001', createdBy:'u1',
      template:ReportTemplate.CUSTOM, dimensions:[ReportDimension.CURRENCY], metrics:['myMetric','rate'] });
    expect(r.metrics).toEqual(['myMetric','rate']);
  });

  it('run() returns COMPLETED status', () => {
    const def = builder.define({ name:'LCR Report', tenantId:'bank-001', createdBy:'u1',
      template:ReportTemplate.LCR, dimensions:[ReportDimension.LEGAL_ENTITY] });
  ...
  it('run history tracks all runs for a report', () => {
    const def = builder.define({ name:'P&L', tenantId:'bank-001', createdBy:'u1',
      template:ReportTemplate.PNL, dimensions:[ReportDimension.TRADER] });
  ...
  it('listReports filters by tenantId', () => {
    builder.define({ name:'R1', tenantId:'bank-001', createdBy:'u1', template:ReportTemplate.LCR, dimensions:[ReportDimension.BOOK] });
    builder.define({ name:'R2', tenantId:'bank-001', createdBy:'u1', template:ReportTemplate.PNL, dimensions:[ReportDimension.TRADER] });
    builder.define({ name:'R3', tenantId:'bank-002', createdBy:'u2', template:ReportTemplate.LCR, dimensions:[ReportDimension.BOOK] });
    expect(builder.listReports('bank-001')).toHaveLength(2);
  });

  it('schedule and delivery configuration is stored', () => {
    const r = builder.define({ name:'Sched', tenantId:'bank-001', createdBy:'u1',
      template:ReportTemplate.CAPITAL, dimensions:[ReportDimension.LEGAL_ENTITY],
      schedule:{ frequency:'DAILY', cronExpression:'0 8 * * 1-5', timezone:'UTC', startDate:'2026-05-01', active:true },
      delivery:{ method:'EMAIL', recipients:['treasury@bank.com'], subjectTemplate:'Capital Report {date}' }
    });
