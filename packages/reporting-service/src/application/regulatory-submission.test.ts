...
describe('RegulatorySubmissionEngine — Sprint 10.4', () => {
  ...
  it('acknowledge transitions status to ACKNOWLEDGED', () => {
    const s = engine.submit({
      tenantId: 'bank-001',
      regulator: RegulatorCode.CBUTT_ALMA,
  ...
  it('reject transitions status to REJECTED with reason', () => {
    const s = engine.submit({
      tenantId: 'bank-001',
      regulator: RegulatorCode.EBA_FINREP,
  ...
  it('getStatus returns submission by ID', () => {
    const s = engine.submit({
      tenantId: 'bank-001',
      regulator: RegulatorCode.BOG_ANNUAL,
  ...
  it('unique IDs across multiple submissions', () => {
    const a = engine.submit({
      tenantId: 'bank-001',
      regulator: RegulatorCode.CBN_MONTHLY,
      reportingPeriod: '2026-01',
      fileSize: 80_000,
    });
    const b = engine.submit({
      tenantId: 'bank-001',
      regulator: RegulatorCode.CBN_MONTHLY,
