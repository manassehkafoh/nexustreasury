...
export const SubmissionStatus = {
  DRAFT:    'DRAFT', SUBMITTED:'SUBMITTED', ACKNOWLEDGED:'ACKNOWLEDGED', REJECTED:'REJECTED',
} as const;
...
export class RegulatorySubmissionEngine {
  ...
  submit(params: { tenantId:string; regulator:RegulatorCode; reportingPeriod:string; fileSize:number; xbrlPackage?:string }): SubmissionRecord {
    ...
    const rec: SubmissionRecord = {
      ...
      submittedAt:new Date().toISOString(), fileSize:params.fileSize, xbrlPackage:params.xbrlPackage,
    };
    this._submissions.set(id, rec); return rec;
  }
  ...
  acknowledge(submissionId:string): SubmissionRecord {
    ...
    const updated = { ...r, status:SubmissionStatus.ACKNOWLEDGED, acknowledgedAt:new Date().toISOString() };
    this._submissions.set(submissionId, updated); return updated;
  }
