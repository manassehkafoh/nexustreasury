...
export interface TreasuryDataContext {
  ...
  readonly limitUtilisation?: Array<{ counterpartyId: string; utilisedPct: number; headroomUSD: number }>;
...
export class TreasuryAIAssistant {
  ...
  async ask(query: AssistantQuery): Promise<AssistantResponse> {
    ...
    const sanitised = this._config.piiRedaction
      ? this._redactPII(query.question)
      : query.question;
  ...
  get metrics() {
    return {
      ...
      successRate:   this._totalQueries > 0
  ...
  private _buildUserMessage(question: string, ctx?: TreasuryDataContext): string {
    ...
    if (ctx.fxPositions?.length) {
      ...
      ctx.fxPositions.forEach(p =>
        lines.push(`- ${p.pair}: Net ${p.netPosition.toLocaleString()} (MTM: USD ${p.mtm.toLocaleString()})`)
      );
    }
    ...
    if (ctx.limitUtilisation?.length) {
      ...
      ctx.limitUtilisation.forEach(l =>
        lines.push(`- CP-${l.counterpartyId}: ${l.utilisedPct.toFixed(1)}% utilised, headroom USD ${l.headroomUSD.toLocaleString()}`)
      );
    }

    if (ctx.lcrRatio !== undefined)  lines.push(`\n### Regulatory Ratios\n- LCR: ${ctx.lcrRatio.toFixed(1)}% (min 100%)`);
  ...
  private _classifyQuery(q: string): QueryCategory {
    ...
    if (/limit|utilisation|counterparty|headroom/.test(lower))  return QueryCategory.LIMIT_UTILISATION;
  ...
  private _parseResponse(
    ...
    return {
      ...
      disclaimers:        ['AI-generated analysis. Verify against source systems before trading decisions.'],
    };
  }

  private _generateFollowUps(category: QueryCategory): string[] {
    const map: Record<QueryCategory, string[]> = {
      FX_EXPOSURE:       ['What is our EUR/USD hedge ratio?', 'Show me FX exposure by subsidiary', 'What is the VaR of our FX book?'],
      LIMIT_UTILISATION: ['Which counterparties are above 80% utilisation?', 'Show limit breach history this month', 'What is our total counterparty credit exposure?'],
      IRRBB_ANALYSIS:    ['What is our EVE sensitivity to a +200bp shock?', 'Show me the NMD repricing assumptions', 'Compare NII sensitivity across 6 standard scenarios'],
      LIQUIDITY_RATIOS:  ['What is our survival horizon under the stressed scenario?', 'Show HQLA composition by asset class', 'What is the contingency funding plan trigger level?'],
      CAPITAL_POSITION:  ['What is our distance to MDA?', 'Show RAROC by business unit', 'What is the impact of a 20% RWA increase on CET1?'],
      TRADE_BLOTTER:     ['What are the top 10 positions by DV01?', 'Show me trades maturing in the next 7 days', 'What is the daily P&L attribution?'],
      PROFITABILITY:     ['Compare NIM vs prior quarter', 'Show RAROC by customer segment', 'What is our cost-to-income ratio trend?'],
      GENERAL:           ['What is our overall risk profile today?', 'Show me key metrics vs targets', 'What regulatory reports are due this week?'],
    };
