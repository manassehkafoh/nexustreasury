...
export class ReportBuilder {
  ...
  define(params: {
    ...
    const def: ReportDefinition = {
      ...
      filters:  params.filters, format: params.format ?? ReportFormat.PDF,
      schedule: params.schedule, delivery: params.delivery,
  ...
  run(reportId: string, triggeredBy: 'SCHEDULE' | 'MANUAL' = 'MANUAL'): ReportRun {
    ...
    const run: ReportRun = {
      ...
      deliveryStatus: report.delivery ? `Delivered via ${report.delivery.method}` : 'Available for download',
    };
    this._runs.set(runId, run);
    return run;
  }

  getReport(id: string): ReportDefinition | undefined { return this._reports.get(id); }
