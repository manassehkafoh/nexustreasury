...
export class FINREPEngine {
  generateReport(bs: FinrepBalanceSheetInput, pl: FinrepPLInput): FinrepReport {
    const totalAssets   = bs.loansAMC + bs.loansNonPerform + bs.sukukFVOCI +
                          bs.derivativesFVPL + bs.cashEquivalents + bs.otherAssets;
    ...
    const nplRatio      = (bs.loansAMC + bs.loansNonPerform) > 0
      ? bs.loansNonPerform / (bs.loansAMC + bs.loansNonPerform) * 100 : 0;
