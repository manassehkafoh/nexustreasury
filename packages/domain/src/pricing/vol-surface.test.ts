...
const EURUSD_SLICES: SVISlice[] = [
  { tau: 0.0833, a: 0.0004, b: 0.015, rho: -0.12, m: 0.001, sigma: 0.018 }, // 1M
  { tau: 0.25, a: 0.0012, b: 0.015, rho: -0.12, m: 0.0005, sigma: 0.018 }, // 3M
  { tau: 0.5, a: 0.0024, b: 0.015, rho: -0.1, m: 0.0003, sigma: 0.018 }, // 6M
  { tau: 1.0, a: 0.0048, b: 0.015, rho: -0.08, m: 0.0002, sigma: 0.018 }, // 1Y
];

const EURUSD_MARKET_QUOTES: VolQuote[] = [
  { tau: 0.0833, atmVol: 0.082, rr25: -0.0025, bf25: 0.0003, forward: 1.0842 },
  { tau: 0.25, atmVol: 0.0785, rr25: -0.0022, bf25: 0.0003, forward: 1.0842 },
  { tau: 1.0, atmVol: 0.075, rr25: -0.0018, bf25: 0.0002, forward: 1.0842 },
];
...
describe('SVIVolatilitySurface', () => {
  ...
  it('vol is positive for all strikes along 1Y expiry slice', () => {
    const strikes = [0.95, 1.0, 1.05, 1.08, 1.1, 1.15, 1.2].map((s) => s * 1.0842);
  ...
  it('surface() grid returns correct dimensions', () => {
    const strikes = [0.95, 1.0, 1.05, 1.1].map((s) => s * 1.0842);
