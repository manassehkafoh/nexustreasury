...
export class BarrierOptionPricer {
  ...
  price(input: BarrierOptionInput): BarrierOptionResult {
    ...
    const { spot: S, strike: K, barrier: H, rebate: R,
            riskFreeRate: r, dividendYield: q, volatility: sigma,
            timeToExpiry: T, barrierType, optionType } = input;
    ...
    if (isKnockedOut) {
      ...
      return {
        price, delta: 0, gamma: 0, vega: 0,
        isKnockedIn: false, isKnockedOut: true,
  ...
  private _computeTerms(
    S: number, K: number, H: number, R: number,
    r: number, q: number, sigma: number, T: number,
    barrierType: BarrierType, optionType: ExoticOptionType,
  ): BarrierTerms {
  ...
  private _computePrice(
    ...
    const A = phi * (S * dfQ * normCDF(phi * x1) - K * dfR * normCDF(phi * (x1 - sigma * Math.sqrt(T))));

    // Component B — vanilla-like term at strike H
    const B = phi * (S * dfQ * normCDF(phi * x2) - K * dfR * normCDF(phi * (x2 - sigma * Math.sqrt(T))));

    // Component C — reflected term at H²/SK
    const mu = Math.pow(H / S, 2 * lambda);
    const C  = phi * (S * dfQ * mu * normCDF(eta * y1) - K * dfR * mu * normCDF(eta * (y1 - sigma * Math.sqrt(T))));

    // Component D — reflected term at H
    const D  = phi * (S * dfQ * mu * normCDF(eta * y2) - K * dfR * mu * normCDF(eta * (y2 - sigma * Math.sqrt(T))));

    switch (barrierType) {
      case 'DOWN_AND_OUT':
        return isCall
          ? (K < H ? A - C + (B - D) * 0 : A - C) // simplified: K≥H
          : (K > H ? (A - B + C - D) : (0));       // put approximation
        // Full Rubinstein-Reiner for down-and-out call (K > H):
        // return A - C;
      case 'DOWN_AND_IN':
        // In-out parity: in + out = vanilla
        return this._vanillaPrice(S, K, T, r, q, sigma, optionType) -
  ...
  private _downOutPrice(
    S: number, K: number, H: number, T: number,
    r: number, q: number, sigma: number, optionType: ExoticOptionType,
    lambda: number, dfR: number, dfQ: number,
  ): number {
    const sqrtT = Math.sqrt(T);
    const x1 = Math.log(S / K) / (sigma * sqrtT) + lambda * sigma * sqrtT;
    const x2 = Math.log(S / H) / (sigma * sqrtT) + lambda * sigma * sqrtT;
    ...
    const phi = optionType === 'CALL' ? 1 : -1;
    return phi * (
      S * dfQ * normCDF(phi * x1) - K * dfR * normCDF(phi * (x1 - sigma * sqrtT))
      - S * dfQ * mu * normCDF(phi * y1) + K * dfR * mu * normCDF(phi * (y1 - sigma * sqrtT))
    );
  }
  ...
  private _upOutPrice(
    ...
    const phi = optionType === 'CALL' ? 1 : -1;
    return phi * (
      S * dfQ * normCDF(phi * x2) - K * dfR * normCDF(phi * (x2 - sigma * sqrtT))
      - S * dfQ * mu * normCDF(phi * y2) + K * dfR * mu * normCDF(phi * (y2 - sigma * sqrtT))
    );
  }
  ...
  private _rawPrice(input: BarrierOptionInput): number {
    ...
    const dfQ = Math.exp(-q * T);
    const terms = this._computeTerms(S, K, H, R, r, q, sigma, T, barrierType, optionType);
    return Math.max(0, this._computePrice(terms, S, K, H, R, r, q, sigma, T, barrierType, optionType));
  }
