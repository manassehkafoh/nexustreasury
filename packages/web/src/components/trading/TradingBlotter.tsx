  ...
  const connect = useCallback((): void => {
    ...
    ws.onmessage = (evt: MessageEvent): void => {
      try {
        ...
        setTrades((prev: BlotterRow[]) => [...incoming, ...prev].slice(0, MAX_BLOTTER_ROWS));
      } catch {
    ...
    ws.onclose = (evt: CloseEvent): void => {
      ...
      if (evt.code !== 1000) {
        const delay =
          RECONNECT_MS[Math.min(retryCountRef.current, RECONNECT_MS.length - 1)] ?? 10_000;
    ...
    <div className="bg-[#071827] border border-[#243558] rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-[#243558]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[#6882A8] text-xs">({trades.length} trades)</span>
        </div>
        <div className="flex items-center gap-2">
          {lastError && <span className="text-xs text-red-400 mr-2">{lastError}</span>}
          <span className={`text-xs font-mono ${statusColor[status]}`}>{status.toUpperCase()}</span>
          ...
          <tbody>
            {trades.length === 0 ? (
              ...
              trades.map((t) => (
                ...
                >
                  <td className="px-3 py-2 font-mono text-[#D4A843] whitespace-nowrap">
                    {t.reference}
                  </td>
                  <td className="px-3 py-2 text-[#EAF0FF]">{t.assetClass}</td>
                  <td
                    className={`px-3 py-2 font-semibold ${t.direction === 'BUY' ? 'text-emerald-400' : 'text-red-400'}`}
                  >
                    {t.direction}
                  </td>
                  ...
                  <td className="px-3 py-2 font-mono text-[#6882A8] whitespace-nowrap">
                    {t.valueDate}
                  </td>
                </tr>
              ))
