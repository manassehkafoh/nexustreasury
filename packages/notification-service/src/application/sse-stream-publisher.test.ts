...
describe('SSEStreamPublisher — Sprint 11.3', () => {
  ...
  it('publish delivers to matching tenant subscriptions', () => {
    ...
    const deliveries = pub.publish({
      type: StreamEventType.POSITION_MTM,
      tenantId: 'bank-001',
      data: { pair: 'EUR/USD', mtm: 54_200_000 },
  ...
  it('publish isolates by tenant (no cross-tenant delivery)', () => {
    ...
    const deliveries = pub.publish({
      type: StreamEventType.LCR_INTRADAY,
      tenantId: 'bank-001',
      data: { lcrRatio: 142.5 },
      timestamp: new Date().toISOString(),
    });
  ...
  it('SSE payload contains event, id, and data fields', () => {
    ...
    const [d] = pub.publish({
      type: StreamEventType.RATE_FEED,
      tenantId: 'bank-001',
      data: { pair: 'EUR/USD', mid: 1.0842 },
      timestamp: new Date().toISOString(),
    });
  ...
  it('filtered subscriptions only receive requested event types', () => {
    ...
    const lcr = pub.publish({
      type: StreamEventType.LCR_INTRADAY,
      tenantId: 'bank-001',
      data: {},
      timestamp: new Date().toISOString(),
    });
    const rate = pub.publish({
      type: StreamEventType.RATE_FEED,
      tenantId: 'bank-001',
      data: {},
      timestamp: new Date().toISOString(),
