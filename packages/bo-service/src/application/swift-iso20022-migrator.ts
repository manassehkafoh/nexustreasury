...
export class SWIFTISO20022Migrator {
  ...
  convertMT103ToPacs008(mt: MT103Fields): MigrationResult<Pacs008> {
    ...
    if (!mt.beneficiaryIBAN)
      errors.push('IBAN missing — pacs.008 requires structured creditor account');
    if (!mt.purposeCode)
      errors.push('WARN: Purpose code (Field 26T) missing — CBPR+ recommends inclusion');
    ...
    return {
      mxMessage: pacs008,
      mtMessageType: MTMessageType.MT103,
      mxMessageType: MXMessageType.PACS008,
      uetr,
  ...
  convertMT202ToPacs009(mt: MT202Fields): MigrationResult<Pacs009> {
    ...
    return {
      mxMessage: pacs009,
      mtMessageType: MTMessageType.MT202,
      mxMessageType: MXMessageType.PACS009,
      uetr,
