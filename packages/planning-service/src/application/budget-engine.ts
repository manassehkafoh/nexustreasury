  ...
  generateReport(budgetId: string): BudgetReport {
    ...
    return {
      ...
      ftpAssessment: {
        ...
        netFTPMarginBps: -20,
        recommendation:
          'Net FTP drain 20bps — extend medium-term funding to reduce wholesale cost.',
      },
      generatedAt: new Date().toISOString(),
    };
  }

  getPlan(budgetId: string): BudgetPlan | undefined {
    return this._plans.get(budgetId);
  }
  listPlans(tenantId: string, fiscalYear?: number): BudgetPlan[] {
    const out: BudgetPlan[] = [];
    this._plans.forEach((p) => {
      if (p.tenantId === tenantId && (!fiscalYear || p.fiscalYear === fiscalYear)) out.push(p);
    });
    return out;
  }
}
