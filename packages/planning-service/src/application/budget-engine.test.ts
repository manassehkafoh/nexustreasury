...
import { BudgetEngine, BudgetScenario, BudgetStatus, BudgetPeriod, type BudgetEntry } from './budget-engine.js';

const ENTRIES: BudgetEntry[] = [
  { businessUnit: 'FX_TRADING',      period: BudgetPeriod.ANNUAL, currency: 'USD', targetNII: 5_000_000, targetNIM: 0.032, nonInterestIncome: 2_000_000, opex: 1_500_000, rwa: 80_000_000,  capitalAllocated: 8_000_000 },
  { businessUnit: 'RETAIL_BANKING',  period: BudgetPeriod.ANNUAL, currency: 'USD', targetNII: 12_000_000, targetNIM: 0.028, nonInterestIncome: 1_000_000, opex: 4_000_000, rwa: 200_000_000, capitalAllocated: 20_000_000 },
  { businessUnit: 'TREASURY',        period: BudgetPeriod.ANNUAL, currency: 'USD', targetNII: 3_000_000,  targetNIM: 0.025, nonInterestIncome: 500_000,   opex: 800_000,  rwa: 50_000_000,  capitalAllocated: 5_000_000 },
];

describe('BudgetEngine — Sprint 9-A (FIS BSM gap closure)', () => {
  let engine: BudgetEngine;
  beforeEach(() => { engine = new BudgetEngine(); });

  it('createBudget returns DRAFT plan', () => {
    const p = engine.createBudget({ tenantId: 'bank-001', fiscalYear: 2027, scenario: BudgetScenario.BASE, entries: ENTRIES });
  ...
  it('budgetId is unique across multiple plans', () => {
    const a = engine.createBudget({ tenantId: 'bank-001', fiscalYear: 2027, scenario: BudgetScenario.BASE, entries: ENTRIES });
    const b = engine.createBudget({ tenantId: 'bank-001', fiscalYear: 2027, scenario: BudgetScenario.STRESS, entries: ENTRIES });
  ...
  it('listPlans returns all plans for tenant', () => {
    engine.createBudget({ tenantId: 'bank-001', fiscalYear: 2027, scenario: BudgetScenario.BASE, entries: ENTRIES });
    engine.createBudget({ tenantId: 'bank-001', fiscalYear: 2027, scenario: BudgetScenario.STRESS, entries: ENTRIES });
    engine.createBudget({ tenantId: 'bank-002', fiscalYear: 2027, scenario: BudgetScenario.BASE, entries: ENTRIES });
    expect(engine.listPlans('bank-001')).toHaveLength(2);
  });

  it('STRESS scenario plan generates higher opex ratio (more conservative)', () => {
    const base = engine.createBudget({ tenantId: 'bank-001', fiscalYear: 2027, scenario: BudgetScenario.BASE, entries: ENTRIES });
    const stressEntries = ENTRIES.map(e => ({ ...e, opex: e.opex * 1.2, targetNII: e.targetNII * 0.85 }));
    const stress = engine.createBudget({ tenantId: 'bank-001', fiscalYear: 2027, scenario: BudgetScenario.STRESS, entries: stressEntries });
