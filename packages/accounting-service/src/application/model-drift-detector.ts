...
export class ModelDriftDetector {
  ...
  private readonly _baselineCDF = [
    0.001, 0.003, 0.006, 0.010, 0.018, 0.028, 0.045, 0.075, 0.130, 0.250, 1.0,
  ];
  ...
  private _baselineECDF(x: number): number {
    for (let i = 1; i < this._baselineCDF.length; i++) {
      if (x <= this._baselineCDF[i]) {
        ...
        const t = (x - this._baselineCDF[i - 1]) / (this._baselineCDF[i] - this._baselineCDF[i - 1]);
        return this._baselineBreaks[i - 1] + t * (this._baselineBreaks[i] - this._baselineBreaks[i - 1]);
      }
    }
    return 1.0;
  }
  ...
  private _findAffectedBands(): string[] {
    const bands: string[] = [];
    const highPD    = this._livePDs.filter(p => p > 0.10).length / this._livePDs.length;
    const veryHighPD = this._livePDs.filter(p => p > 0.30).length / this._livePDs.length;
    if (highPD > 0.20)     bands.push('BB+ and below (>10% PD)');
