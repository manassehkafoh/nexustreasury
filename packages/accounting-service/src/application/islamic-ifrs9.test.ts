...
import { IslamicIFRS9Extension, IslamicInstrumentType, IFRS9Classification } from './islamic-ifrs9.js';

const ext = new IslamicIFRS9Extension();
const BASE = { outstandingAmount: 1_000_000, currency: 'USD', profitRate: 0.055, tenorYears: 3, daysPastProfit: 0, isNonPerforming: false, recoveryRate: 0.40, pd12Month: 0.02, pdLifetime: 0.08 };
...
describe('IslamicIFRS9Extension — Sprint 9.3', () => {
  ...
  it('Stage 2 for DPP >= 30', () => {
    const r = ext.calculateECL({ ...BASE, instrumentType: IslamicInstrumentType.IJARA, daysPastProfit: 35 });
    expect(r.stage).toBe(2);
  });
  it('Stage 3 for non-performing', () => {
    const r = ext.calculateECL({ ...BASE, instrumentType: IslamicInstrumentType.MURABAHA, isNonPerforming: true, daysPastProfit: 95 });
  ...
  it('Stage 3 ECL > Stage 1 ECL (lifetime PD > 12M PD)', () => {
    ...
    const s3 = ext.calculateECL({ ...BASE, instrumentType: IslamicInstrumentType.MURABAHA, isNonPerforming: true, daysPastProfit: 95 });
