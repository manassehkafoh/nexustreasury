...
export type IslamicInstrumentType = (typeof IslamicInstrumentType)[keyof typeof IslamicInstrumentType];
...
export class IslamicIFRS9Extension {
  ...
  private _assignStage(input: IslamicECLInput): { stage: 1|2|3; rationale: string } {
    if (input.isNonPerforming || input.daysPastProfit >= 90) {
      return { stage: 3, rationale: `Non-performing Islamic asset: DPP=${input.daysPastProfit}d. Stage 3 — lifetime ECL.` };
    }
    if (input.daysPastProfit >= 30 || input.pd12Month > 0.05) {
      return { stage: 2, rationale: `Significant increase in credit risk: DPP=${input.daysPastProfit}d or PD>5%. Stage 2 — lifetime ECL.` };
    }
