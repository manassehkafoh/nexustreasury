...
import { BloombergBPIPEAdapter, CircuitState, type BPIPEConfig } from './bloomberg-bpipe-adapter.js';
...
export class AdaptiveMarketDataAdapter implements MarketDataAdapter {
  ...
  constructor(config: AdaptiveAdapterConfig) {
    ...
    this._fallback  = config.useMockFallback
      ? new MockRateAdapter()
      : new MockRateAdapter(); // Refinitiv RDP would go here in production
  ...
  private _startCircuitMonitor(): void {
    this._probeTimer = setInterval(() => {
      ...
      } else if (this._bloomberg.circuitState === CircuitState.CLOSED && this._active === 'FALLBACK') {
  ...
  private _restoreBloomberg(): void {
    ...
    this._events.push(event);
    this._fallback.disconnect().catch(() => {/* ignore */});
  }
}
