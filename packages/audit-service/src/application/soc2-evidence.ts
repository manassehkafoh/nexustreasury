...
export class SOC2EvidenceCollector {
  ...
  collectAutomatedEvidence(): EvidenceItem[] {
    ...
    const items: EvidenceItem[] = [
      // CC6 — Access Controls
      {
        ...
        criteria: TrustCriteria.CC6_ACCESS_CONTROL,
        control: 'CC6.1 — Logical access controls',
        description: 'Keycloak OAuth2/OIDC enforced on all service endpoints',
        source: 'AUTOMATED',
        ...
        drataControlId: 'CC6.1-001',
        vantaTestId: 'VANTA-IAM-001',
      },
      {
        evidenceId: randomUUID(),
        criteria: TrustCriteria.CC6_ACCESS_CONTROL,
        control: 'CC6.3 — Role-based access control',
        description: 'RBAC enforced; least-privilege principle verified',
        source: 'AUTOMATED',
        status: EvidenceStatus.COLLECTED,
        collectedAt: now,
        expiresAt: exp,
      ...
      {
        evidenceId: randomUUID(),
        criteria: TrustCriteria.CC7_OPERATIONS,
        control: 'CC7.2 — Monitoring and alerting',
        description: 'Prometheus+Grafana SLO monitoring with PagerDuty alerting',
        source: 'AUTOMATED',
        status: EvidenceStatus.COLLECTED,
        collectedAt: now,
        expiresAt: exp,
        data: {
          ...
          incidentResponseSLAMin: 15,
          onCallRotation: true,
        },
        drataControlId: 'CC7.2-001',
        vantaTestId: 'VANTA-MON-001',
      },
      {
        evidenceId: randomUUID(),
        criteria: TrustCriteria.CC7_OPERATIONS,
        control: 'CC7.4 — Incident response',
        description: 'DR runbook automation with RTO/RPO measurement',
        source: 'AUTOMATED',
        status: EvidenceStatus.COLLECTED,
        collectedAt: now,
        expiresAt: exp,
      ...
      {
        evidenceId: randomUUID(),
        criteria: TrustCriteria.CC8_CHANGE_MGMT,
        control: 'CC8.1 — Change authorization',
        description: 'GitOps ArgoCD pipeline with PR review enforcement',
        source: 'AUTOMATED',
        status: EvidenceStatus.COLLECTED,
        collectedAt: now,
        expiresAt: exp,
        data: {
          cicdProvider: 'GitHub Actions + ArgoCD',
          branchProtection: true,
          requiredReviewers: 2,
          deploymentsLast90d: 47,
        },
        drataControlId: 'CC8.1-001',
        vantaTestId: 'VANTA-CM-001',
      },
      // CC9 — Risk Mitigation
      {
        evidenceId: randomUUID(),
        criteria: TrustCriteria.CC9_RISK_MGMT,
        control: 'CC9.2 — Vendor risk management',
        description: 'Third-party dependency audit: 0 prod CVEs',
        source: 'AUTOMATED',
        status: EvidenceStatus.COLLECTED,
        collectedAt: now,
        expiresAt: exp,
      ...
      {
        evidenceId: randomUUID(),
        criteria: TrustCriteria.CC1_CONTROL_ENV,
        control: 'CC1.4 — Background checks',
        description: 'Employee background check policy — PENDING manual evidence',
        source: 'MANUAL',
  ...
  generateReadinessReport(period: string): AuditReadinessReport {
    ...
    const findings: string[] = [
      ...this._evidence
        .filter((e) => e.status === EvidenceStatus.PENDING)
