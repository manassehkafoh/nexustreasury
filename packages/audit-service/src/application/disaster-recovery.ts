...
export class DisasterRecoveryOrchestrator {
  ...
  recordHealthCheck(
    ...
    const failures = latencyMs > LATENCY_THRESHOLD_MS || errorRatePct > 5
      ? (prev?.consecutiveFailures ?? 0) + 1 : 0;

    const status: RegionStatus =
      failures === 0             ? RegionStatus.HEALTHY   :
      failures < FAILURE_THRESHOLD ? RegionStatus.DEGRADED :
      RegionStatus.UNREACHABLE;

    this._regionHealth.set(region, {
      region, status, latencyMs, errorRatePct,
  ...
  runDRTest(scenario: string): DRTestResult {
    ...
    if (rtoMeasured > RTO_TARGET_MS) findings.push(`RTO exceeded target by ${Math.round((rtoMeasured - RTO_TARGET_MS)/1000)}s`);
    if (rpoMeasured > RPO_TARGET_MS) findings.push(`RPO exceeded target by ${Math.round((rpoMeasured - RPO_TARGET_MS)/1000)}s`);

    return {
      testId, testDate: new Date().toISOString(), scenario,
  ...
  getRegionHealth(): RegionHealth[] { return Array.from(this._regionHealth.values()); }
  getFailoverLog():  FailoverEvent[] { return [...this._failoverLog]; }
  get primaryRegion(): string { return this._primaryRegion; }
  ...
  private _initiateFailover(trigger: FailoverTrigger): FailoverEvent {
    ...
    const event: FailoverEvent = {
      eventId:      `FO-${Date.now()}`,
      trigger, fromRegion: this._primaryRegion, toRegion,
      initiatedAt:  initAt,
    ...
    this._standbyRegions = this._standbyRegions.filter(r => r !== toRegion).concat(event.fromRegion);
    return event;
  }

  private _buildRunbookSteps(): RunbookStep[] {
    return [
      { stepId:1, name:'Health probe confirmation',    command:'kubectl get pods -n nexustreasury --all-namespaces | grep -v Running',    expectedMs: 30_000,  status:'PENDING' },
      { stepId:2, name:'DNS failover (Route53/Azure)', command:'az network dns record-set a update --ttl 30 ...',                          expectedMs: 60_000,  status:'PENDING' },
      { stepId:3, name:'PostgreSQL promote standby',   command:'kubectl exec -n postgres pg-standby -- pg_ctl promote',                    expectedMs: 120_000, status:'PENDING' },
      { stepId:4, name:'Kafka consumer rebalance',     command:'kubectl rollout restart deployment/kafka-consumer -n nexustreasury',       expectedMs: 45_000,  status:'PENDING' },
      { stepId:5, name:'Service health validation',    command:'curl -sf https://nexustreasury-dr.bank.com/health | jq .status',          expectedMs: 30_000,  status:'PENDING' },
      { stepId:6, name:'PagerDuty alert resolution',   command:'pd-cli event send --routing-key $PD_KEY --action resolve ...',             expectedMs: 5_000,   status:'PENDING' },
    ];
  }

  private _buildPagerDutyAlert(
    trigger: FailoverTrigger, toRegion: string, rtoMs: number,
  ): PagerDutyAlert {
    return {
