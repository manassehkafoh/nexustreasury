...
const STORAGE_COST_PER_GIB_MO = 0.10;    // EBS gp3
...
export class FinOpsCostTracker {
  ...
  addEntry(entry: ServiceCostEntry): void { this._entries.push(entry); }
  ...
  generateTenantReport(tenantId: string, period: string): TenantCostReport {
    const entries = this._entries.filter(e => e.tenantId === tenantId &&
      e.periodStart.startsWith(period.slice(0, 7)));
    ...
    for (const e of entries) {
      ...
      serviceMap.set(e.service, {
        cpu: prev.cpu + cpuC, mem: prev.mem + memC,
        stor: prev.stor + stoC, net: prev.net + netC,
      });
    }

    let totalCPU=0, totalMem=0, totalStor=0, totalNet=0;
    serviceMap.forEach(v => { totalCPU+=v.cpu; totalMem+=v.mem; totalStor+=v.stor; totalNet+=v.net; });
    ...
    const breakdown: ServiceCostLine[] = [...serviceMap.entries()]
      .map(([svc, v]) => ({
        ...
        pctOfTotal: total > 0 ? parseFloat(((v.cpu+v.mem+v.stor+v.net)/total*100).toFixed(1)) : 0,
      }))
  ...
  getAllTenantsSummary(period: string): { tenantId: string; totalCostUSD: number }[] {
    ...
    return tenants
      .map(t => ({ tenantId:t, totalCostUSD: this.generateTenantReport(t, period).totalCostUSD }))
