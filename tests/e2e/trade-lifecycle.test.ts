...
import { Trade, AssetClass, Money, BusinessDate, TradeDirection } from '@nexustreasury/domain';
import {
  SanctionsScreeningService,
  SanctionsResult,
} from '../../packages/trade-service/src/application/services/sanctions-screening.service.js';
import { VaRCalculator } from '../../packages/risk-service/src/application/var/var-calculator.js';
import {
  FRTBSAEngine,
  FRTBRiskClass,
} from '../../packages/risk-service/src/application/frtb/frtb-sa-engine.js';
...
import {
  ...
  LifecycleEventType,
} from '../../packages/bo-service/src/application/corporate-actions/corporate-actions.service.js';
import {
  createAuditRecord,
  verifyAuditRecord,
  AuditCategory,
  AuditSeverity,
} from '../../packages/audit-service/src/domain/audit-record.js';
import {
  AuditEventRouter,
  InMemoryAuditRepository,
} from '../../packages/audit-service/src/application/audit-event-router.js';
...
import {
  NMDModellingService,
  NMDProductType,
} from '../../packages/alm-service/src/application/nmd-modelling.js';
...
describe('E2E 1: Sanctions Screening', () => {
  ...
  it('MATCH: test sentinel entity flagged (exact name match)', async () => {
    ...
    const r = await strictScreener.screen({
      counterpartyId: 'bad',
      name: '__TEST_SANCTIONED_ENTITY__',
    });
  ...
  it('rejects trade when pre-deal check fails', () => {
    expect(() =>
      Trade.book({
        ...
        counterpartyId: CP_ID as any,
        instrumentId: 'GBPUSD' as any,
        bookId: 'fx-desk' as any,
        traderId: 'trader-01' as any,
        notional: Money.of(500_000, 'GBP'),
        price: 1.264,
        tradeDate: BusinessDate.fromDate(TODAY),
        valueDate: BusinessDate.fromDate(new Date('2026-04-11')),
...
describe('E2E 3: IFRS9 Classification', () => {
  ...
  it('FX Forward → FVPL_MANDATORY', () =>
    ...
    ).toBe(IFRS9Category.FVPL_MANDATORY));
  it('Bond HTC → AMORTISED_COST', () =>
    expect(
      cls.classify({
    ...
    ).toBe(IFRS9Category.AMORTISED_COST));
  it('Bond HTC+Sell → FVOCI', () =>
    expect(
      cls.classify({
        assetClass: AssetClass.FIXED_INCOME,
        instrumentType: 'BOND',
    ...
    ).toBe(IFRS9Category.FVOCI));
  it('tenant override trumps all', () =>
    expect(
      cls.classify({
        assetClass: AssetClass.FIXED_INCOME,
        instrumentType: 'BOND',
...
describe('E2E 4: ECL Calculation', () => {
  ...
  it('defaulted loan (120 DPD) → Stage 3', () => {
    ...
    expect(r.ecl).toBeGreaterThan(r.ead * 0.5);
  });
});
      ...
      entries: [
        {
          ...
          amount: 1_000_000,
          currency: 'EUR',
          reference: 'FX-E2E-001',
          description: 'EUR settlement',
          nostroAccount: NOSTRO,
        },
      ],
      expectedFlows: [
      ...
      entries: [
        {
          ...
          amount: 999_000,
          currency: 'EUR',
          reference: 'FX-E2E-002',
          description: 'Short settlement',
          nostroAccount: NOSTRO,
        },
      ],
      expectedFlows: [
...
describe('E2E 7: VaR Engine', () => {
  it('HS-VaR: positive, √10-scaled, ES ≥ VaR', async () => {
    const history = Array.from({ length: 250 }, (_, i) => ({
      ...
      pnl: (Math.random() - 0.5) * 200_000,
      currency: 'USD',
    }));
    history[5]!.pnl = -1_500_000;
    history[12]!.pnl = -2_000_000;
...
describe('E2E 11: NMD + Regulatory Reporting', () => {
  it('retail NMD: 3% LCR runoff, 70% core', () => {
    const p = new NMDModellingService().project({
  ...
  it('IRRBB outlier: -$80M EVE on $500M Tier1 = 16% > 15% threshold', () => {
    const r = new RegulatoryReportingService().buildIRRBBReport({
      ...
      niiSensitivity200Up: -20_000_000,
...
describe('E2E 12: Corporate Actions', () => {
  ...
  const BOND = {
    ...
    counterpartyId: CP_ID,
    isCorporateClient: false,
    maturityDate: new Date('2027-04-09'),
  };
